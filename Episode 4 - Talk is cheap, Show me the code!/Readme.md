...
In this episode, we are building a Food Ordering Website UI and exploring the power of React components and props.

---

## Props in React

**Props** are the way we pass data to React components, similar to passing arguments to a function. React automatically converts the passed props into an object that the component can use.

### Basic Props Usage

```jsx
const RestaurantCard = (props) => {
  console.log(props); // {resName: 'Meghana Foods', cuisine: 'North Indian, South Indian, Chinese'}
  ...
  return (
    <div className="res-card">
      ...
      <div className="res-desc">
        ...
        <div className="res-cuisine">North Indian, South Indian, Chinese</div>
        <div className="res-rating">4.4 stars</div>
        <div className="res-delivery">30 mins</div>
      </div>
    </div>
  );
};
...
There are multiple ways to destructure props in React components. Here are two common approaches:

### Method 1: Direct Destructuring in Parameters
...
### Method 2: Destructuring Inside Component
...
**Config-Driven UI** is a powerful pattern where the entire user interface is driven by configuration data. This approach makes applications:

- **Dynamic**: UI changes based on data
- **Scalable**: Easy to add new features
- **Maintainable**: Centralized control over UI behavior
- **Flexible**: Different configurations for different users/regions

Instead of hardcoding UI elements, we define them through data structures and let React render them dynamically.

---

## Key Takeaways

- Props are React's way of passing data between components
- Destructuring makes prop usage cleaner and more readable
- Config-driven UI enables dynamic and scalable applications
- React components become reusable building blocks when properly designed

---

# Rendering UI using Swiggy API 🍕

## Building a Food Ordering App with Real Restaurant Data

In this implementation, we're creating a dynamic restaurant card component that uses real data from the Swiggy API to render restaurant information.

---

## Application Structure 🏗️

Our food ordering app follows a well-organized component structure:
...
Here's the actual restaurant data structure from Swiggy API:
...
### Dynamic Restaurant Card Component
  ...
  return (
    <div className="res-card">
      ...
      <div className="res-desc">
        ...
        <div className="res-rating">{resData.info.avgRating} stars</div>
        <div className="res-delivery">{resData.info.costForTwo}</div>
      </div>
    </div>
  );
};
...
Whenever we are looping over something, we need to pass key to the component. It is because if a new card came, React doesn't know where it should be placed. Therefore, it re renders the whole UI again.

So, we pass key so that react identify where should it be placed.

Some people also use key as index. But it is a bad practice.
